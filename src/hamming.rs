use std::{
    f32::consts::E, marker::PhantomData
};
use halo2_proofs::{
    arithmetic::Field, 
    dev::{metadata::Constraint, MockProver},
...
trait Instructions<F: Field>: Chip<F> {
    ...
    fn load_private_and_check_binary(&self, layouter: impl Layouter<F>, column: usize, value: [Value<F>; BINARY_SIZE]) -> Result<Vec<Self::Num>, Error>;

    fn xor(&self, layouter: impl Layouter<F>, a: Self::Num, b: Self::Num) -> Result<Self::Num, Error>;

    fn accumulator(&self, layouter: impl Layouter<F>, values: [Self::Num; BINARY_SIZE]) -> Result<Self::Num, Error>;

    fn expose_public(&self, layouter: impl Layouter<F>, num: Self::Num) -> Result<(), Error>;
} 


pub struct HammingChip<F: Field> {

    config: HammingConfig,
    _marker: PhantomData<F>,
}

impl<F: Field> Chip<F> for HammingChip<F> {
...
impl <F:Field> HammingChip<F> {
    ...
    fn configure(
        ...
        meta.create_gate("is binary left", |meta| {
            ...
            let l_value = meta.query_advice(advice[0], Rotation::cur());

            vec![
                s_binary_l*(l_value.clone() * (l_value.clone() - Expression::Constant(F::ONE))),
            ]
        });

        meta.create_gate("is right left", |meta| {
            let s_binary_r = meta.query_selector(s_binary_l);
            let r_value = meta.query_advice(advice[1], Rotation::cur());

            vec![
                s_binary_r*(r_value.clone() * (r_value - Expression::Constant(F::ONE))),
            ]
        });
        ...
        meta.create_gate("xor", |meta| {
            ...
            let l_value = meta.query_advice(advice[0], Rotation::cur());
            let r_value = meta.query_advice(advice[1], Rotation::cur());
            let xor_value = meta.query_advice(advice[2], Rotation::cur());

            vec![
                s_xor*(l_value.clone() + r_value.clone() - Expression::Constant(F::ONE.double())*l_value.clone()*r_value.clone() - xor_value.clone()),
            ]
        });
        ...
        meta.create_gate("accumulator", |meta| {
            ...
            let final_sum = (0..BINARY_SIZE)
                                .map(|i| meta.query_advice(advice[2], Rotation((i as i32) - (BINARY_SIZE as i32))))
                                .fold(Expression::Constant(F::ZERO), |acc, x| acc + x);

            let accumulator_value = meta.query_advice(advice[2], Rotation::cur());

            vec![
                s_accumulator*(final_sum - accumulator_value.clone())
            ]
        });
...
impl<F: Field> Instructions<F> for HammingChip<F>{
    type Num = Number<F>;

    fn load_private_and_check_binary(&self, mut layouter: impl Layouter<F>, column: usize, value: [Value<F>; BINARY_SIZE]) -> Result<Vec<Self::Num>, Error> {
        
        let config = self.config();

        layouter.assign_region(|| "left right cloum", |mut region| {
            value
            .iter()
            .enumerate()
            .map(|(i, val)| {
                ...
                region.assign_advice(||"assign private val", 
                config.advice[column], i, || *val)
                .map(Number)
            })
            .collect()
            // / .collect() method is used to collect the results of these map operations into a Vec
        })
    ...
    fn xor(&self, mut layouter: impl Layouter<F>, a: Self::Num, b: Self::Num) -> Result<Self::Num, Error> {
        let config = self.config();
        layouter.assign_region(|| "assign xor values", |mut regoin| {
            
            config.s_xor.enable(&mut regoin, 0);

            ///Two questions:
            /// 1. why 0 offset always? why cant we pass i as offset?
            /// 2. How copy advice from a and b ? 
            /// Answer2: so basically a: Self::Num and Number =Num and Number.0 is AssignedCell<F,F> and AssignedCell<F,F> has copy_advice method
            let a_value = a.0.copy_advice(|| "left val", &mut regoin, config.advice[0], 0)?;
            let b_value = b.0.copy_advice(|| "right val", &mut regoin, config.advice[1], 0)?;

            let xor_value = a_value.value().zip(b_value.value()).map(|(a,b)| if *a == *b {F::ZERO} else {F::ONE});

            regoin.assign_advice(|| "assign xor value", config.advice[2], 0, || xor_value).map(Number)


        })
    }

    fn accumulator(&self, mut layouter: impl Layouter<F>, values: [Self::Num; BINARY_SIZE]) -> Result<Self::Num, Error> {
        let config = self.config();

        layouter.assign_region(||"assign accumulator", |mut region| {
            config.s_accumulator.enable(&mut region, 0);

            for (i, value) in values.iter().enumerate() {
                (*value).0.copy_advice(||format!("output [{}]", i), &mut region, config.advice[2], i)?;
            }

            let accumulation = values.iter().map(|n| n.0.value().copied()).fold(Value::known(F::ZERO), |acc, x| acc + x);

            region.assign_advice(|| "assign accumulator value", config.advice[2], BINARY_SIZE, || accumulation).map(Number)
        })
    }
...
pub struct HammsterCircuit<F: Field> {
    a: [Value<F>; BINARY_SIZE],
    b: [Value<F>; BINARY_SIZE],
}
...
impl<F: Field> Circuit<F> for HammsterCircuit<F> {
    ...
    fn configure(meta: &mut ConstraintSystem<F>) -> Self::Config {
        ...
        HammingChip::configure(meta, advice, instance)
    }

    fn synthesize(&self, config: Self::Config, mut layouter: impl Layouter<F>) -> Result<(), Error> {
        let hammster_chip = HammingChip::<F>::construct(config);
        ...
        let xor_results: Vec<Number<F>> = (0..BINARY_SIZE)
            .map(|i| {
...
pub fn empty_circuit() -> HammsterCircuit<Fp> {
    HammsterCircuit {
        a: [Value::unknown(); BINARY_SIZE],
        b: [Value::unknown(); BINARY_SIZE],
    }
}
...
pub fn create_circuit(a: Vec<u64>, b: Vec<u64>) -> HammsterCircuit<Fp> {
    ...
    let a_vec: [Value<Fp>; BINARY_SIZE] = a
        .clone()
        .iter()
    ...
    let b_vec: [Value<Fp>; BINARY_SIZE] = b
        .clone()
        .iter()
        .map(|f| Value::known(Fp::from(*f)))
        .collect::<Vec<Value<Fp>>>()
